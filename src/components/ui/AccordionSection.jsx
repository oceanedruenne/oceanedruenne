  ...
  if (!isMobile) {
    return (
      <div style={{ display: "flex", gap: "48px", alignItems: "flex-start" }}>
            ...
            <FadeIn key={i} delay={i * 0.08}>
              ...
              >
                {item.company
                  ...
                  : item.school.split("–")[0].trim().split(" ")[0]}
              </button>
            </FadeIn>
          ))}
        </div>
        ...
        {/* Content panel */}
        <div key={`panel-${activeIdx}`} style={{ flex: 1 }}>
          {renderContent(items[activeIdx])}
        </div>
      </div>
    );
  }
      ...
      {items.map((item, i) => {
        ...
        return (
          <FadeIn key={i} delay={i * 0.06}>
            <div
              style={{
                border: `1px solid ${isOpen ? accentColor + "60" : COLORS.border}`,
                borderRadius: "8px",
                overflow: "hidden",
              ...
              >
                <div style={{ textAlign: "left" }}>
                  <div style={{ fontSize: "15px", fontWeight: 600 }}>
                    {item.company || item.school?.split("–")[0].trim()}
                  </div>
